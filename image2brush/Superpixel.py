# import the necessary pacckages
...
def segment_colorfulness(image, mask):
    # split the image into respective RGB components, then mask
    ...
    # compute the mean and standard devidation of both `rg` and `yb`
    # then combine them
...
ap.add_argument("-i", "--image", required=True, help="pah to input image")
ap.add_argument("-s", "--segments", type=int, default=1000, help="# of superpixels")
...
for v in np.unique(segments):
    ...
    vis[segments == v] = C
    print(C)
# scale the visualozation image from an unrestricted floating point
# to unsigned 8-bit integer array so we can use it eith OpenCV and
...
cv2.imwrite('superpixel.png', output)